...
public class CreateWishlistAsyncTest
{
    ...
    public async Task CreateWishlistAsync_Should_CreateWishlist_And_ReturnSuccess()
    {
        ...
        // Act
        Result result = await _wishlistService.CreateWishlistAsync(userId, CancellationToken.None);

        // Assert
        result.IsSuccess.ShouldBeTrue();

        await _wishlistRepositoryMock.Received(1)
            .AddAsync(Arg.Is<Wishlist>(w => w.UserId == userId), Arg.Any<CancellationToken>());
    }
}

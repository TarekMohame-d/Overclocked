...
public class GetWishlistItemsAsyncTest
{
    ...
    [Fact]
    public async Task GetWishlistItemsAsync_Should_ThrowException_When_WishlistNotFound()
    {
        // Arrange
    ...
    public async Task GetWishlistItemsAsync_When_WishlistExists_Should_ReturnItems()
    {
        ...
        {
            new() { WishlistId = wishlist.Id, ProductId = product.Id, Product = product }
        };
        ...
        IQueryable<WishlistItem> mockQueryable = wishlistItems.BuildMock();
        _wishlistItemRepositoryMock.Query()
            .Returns(mockQueryable);

        // Act
        Result<IEnumerable<WishlistItemResponse>> result = await _wishlistService
            .GetWishlistItemsAsync(userId, CancellationToken.None);

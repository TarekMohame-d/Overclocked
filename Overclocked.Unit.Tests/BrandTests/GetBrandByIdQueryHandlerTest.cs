...
public class GetBrandByIdQueryHandlerTest
{
    ...
    public GetBrandByIdQueryHandlerTest()
    {
        ...
        _getBrandByIdQueryHandler = new GetBrandByIdQueryHandler(_brandRepositoryMock);
    }
    ...
    public async Task GetBrandQueryHandler_Should_ReturnBrand_When_BrandExists()
    {
        ...
        var query = new GetBrandByIdQuery { Id = BrandId.Create(brandId) };

        Brand brand = new BrandFaker().Generate();
        BrandResponse brandDto = brand.ToDto();

        _brandRepositoryMock.GetByIdAsync(
            Arg.Any<BrandId>(),
            Arg.Any<CancellationToken>())
            .Returns(brand);

        // Act
        Result<BrandResponse> result = await _getBrandByIdQueryHandler.Handle(query, CancellationToken.None);

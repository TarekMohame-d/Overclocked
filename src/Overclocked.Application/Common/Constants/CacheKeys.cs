...
public static class CacheKeys
{
    ...
    public static string ReviewSet(string productId) => $"reviews:productId={productId}";

    public static string PaymentMethods => "paymentMethods";

    public static string OrderPaged(int page, int pageSize, string direction, string userId, string year) =>
        $"orders:page={page}:size={pageSize}:direction={direction}:userId={userId}:year={year}";

    public static string OrderSet(string userId) => $"orders:userId={userId}";
}

...
public class EmployeeActivityLogConfigurations : IEntityTypeConfiguration<EmployeeActivityLog>
{
    public void Configure(EntityTypeBuilder<EmployeeActivityLog> builder)
    {
        ...
        builder
            .Property(e => e.Id)
            .ValueGeneratedNever()
            .HasConversion(id => id.Value, value => EmployeeActivityLogId.Create(value))
            .IsRequired();

        builder.Property(e => e.EmployeeId).HasConversion(id => id.Value, value => UserId.Create(value)).IsRequired();
        ...
        // Relationships
        builder.HasOne<User>().WithMany().HasForeignKey(e => e.EmployeeId).OnDelete(DeleteBehavior.Restrict);

...
public class WishlistConfiguration : IEntityTypeConfiguration<Wishlist>
{
    public void Configure(EntityTypeBuilder<Wishlist> builder)
    {
        ...
        builder.Property(w => w.UserId).HasConversion(id => id.Value, value => UserId.Create(value)).IsRequired();

        // Relationships
        builder.HasOne<User>().WithOne().HasForeignKey<Wishlist>(w => w.UserId).OnDelete(DeleteBehavior.Cascade);
    ...
    private static void ConfigureWishlistItems(EntityTypeBuilder<Wishlist> builder) =>
        builder.OwnsMany(
            ...
            wiBuilder =>
            {
                ...
                wiBuilder.Property<WishlistId>("WishlistId").HasColumnName("wishlist_id");

                wiBuilder
                    .Property(wi => wi.ProductId)
                    .HasConversion(id => id.Value, value => ProductId.Create(value))
                    .IsRequired();

                wiBuilder.HasKey("WishlistId", "ProductId");

                wiBuilder.HasOne<Product>().WithMany().HasForeignKey(wi => wi.ProductId).OnDelete(DeleteBehavior.Restrict);
            }
        );
}

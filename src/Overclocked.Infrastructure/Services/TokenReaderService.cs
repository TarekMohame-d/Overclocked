...
{
    public (Guid userId, Guid deviceId)? GetUserIdAndDeviceIdFromToken(string accessToken)
    {
        ...
        if (!Guid.TryParse(claims[ClaimsConstants.DeviceId], out Guid parsedDeviceId))
            return null;

        return (userId, parsedDeviceId);
    }

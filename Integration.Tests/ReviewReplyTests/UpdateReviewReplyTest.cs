...
public class UpdateReviewReplyTest(CustomWebApplicationFactory factory) : IAsyncLifetime
{
    ...
    public async Task Update_Should_UpdateAndReturnSuccess_When_DataIsValid()
    {
        ...
        HttpResponseMessage response = await _client
            .PutAsync(ReviewReplyRoutes.Update.Replace("{productId:guid}", product.Id.ToString()).Replace("{reviewId:guid}", review.Id.ToString()).Replace("{replyId:guid}", review.ReviewReply!.Id.ToString()), form);
        ...
        reviewReplyDb.Reply.ShouldBe("new Reply");
    ...
    {
        var payload = new { Reply = "new Reply" };

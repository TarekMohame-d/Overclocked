...
public class UpdateReviewReplyTest(CustomWebApplicationFactory factory) : IAsyncLifetime
{
    ...
    public async Task Update_Should_UpdateAndReturnSuccess_When_DataIsValid()
    {
        ...
        var token = CustomWebApplicationFactory.GenerateJwtToken(userId: admin.Id.Value.ToString(), role: "Admin");
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

        // Act
        HttpResponseMessage response = await _client
            .PutAsync(ReviewReplyRoutes.Update.Replace("{productId:guid}", product.Id.Value.ToString()).Replace("{reviewId:guid}", review.Id.Value.ToString()).Replace("{replyId:guid}", review.ReviewReply!.Id.Value.ToString()), form);
        ...
        reviewDb.ReviewReply.Reply.ShouldBe("New Reply");
    ...
    public async Task Update_Should_ReturnFailure_When_Unauthorized()
    {
        ...
        var token = CustomWebApplicationFactory.GenerateJwtToken(userId: admin2.Id.Value.ToString(), role: "Admin");
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

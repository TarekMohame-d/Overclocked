...
public class CreateReviewReplyTest(CustomWebApplicationFactory factory) : IAsyncLifetime
{
    ...
    public async Task Create_Should_CreateAndReturnSuccess_When_DataIsValid()
    {
        ...
        HttpResponseMessage response = await _client
            .PostAsync(ReviewReplyRoutes.Create.Replace("{productId:guid}", product.Id.Value.ToString()).Replace("{reviewId:guid}", review.Id.Value.ToString()), form);
    ...
    public async Task Create_Should_ReturnFailure_When_ReviewAlreadyReply()
    {
        ...
        // Act
        HttpResponseMessage response = await _client
            .PostAsync(ReviewReplyRoutes.Create.Replace("{productId:guid}", product.Id.Value.ToString()).Replace("{reviewId:guid}", review.Id.Value.ToString()), form1);

        HttpResponseMessage response2 = await _client
            .PostAsync(ReviewReplyRoutes.Create.Replace("{productId:guid}", product.Id.Value.ToString()).Replace("{reviewId:guid}", review.Id.Value.ToString()), form2);

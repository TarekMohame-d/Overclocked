...
// namespace Overclocked.Api.Middleware;
...
//     public QueryParameterValidationMiddleware(RequestDelegate next, ILogger<QueryParameterValidationMiddleware> logger)
//     {
//         _next = next;
//         _logger = logger;
//     }
...
//         var duplicateParams = context.Request.Query.Where(q => q.Value.Count > 1).Select(q => q.Key).ToList();

//         if(duplicateParams.Count != 0)
//         {
//             _logger.LogWarning("Duplicate query parameters detected: {Parameters}", string.Join(", ", duplicateParams));

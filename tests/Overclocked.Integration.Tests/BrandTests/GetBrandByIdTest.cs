...
public class GetBrandByIdTest(IntegrationTestWebAppFactory fixture) : IAsyncLifetime
{
    ...
    public async Task GetById_Should_ReturnBrandFromDatabase_When_CacheMiss()
    {
        ...
        result.Id.ShouldBe(brand.Id.Value);
    ...
    public async Task GetById_Should_ReturnBrandFromCache_When_CacheHit()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.GetAsync(BrandRoutes.GetById.Replace("{id:guid}", brandDto.Id.ToString()));
    ...
    public async Task GetById_Should_ReturnConsistentResults_When_CalledConcurrently()
    {
        ...
        var ids = brands.Select(x => x.Id.Value).ToList();
        ...
        for (var i = 0; i < ConcurrentCalls; i++)
        {
            ...
            Task<HttpResponseMessage> task = _client.GetAsync(BrandRoutes.GetById.Replace("{id:guid}", randomId.ToString()));
            tasks.Add(task);
        }

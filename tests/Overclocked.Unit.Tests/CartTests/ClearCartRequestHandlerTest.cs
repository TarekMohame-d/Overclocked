...
public class ClearCartRequestHandlerTest
{
    private readonly ICartRepository _cartRepositoryMock;
    private readonly IUnitOfWork _unitOfWorkMock;
    private readonly ClearCartRequestHandler _clearCartItemRequestHandler;

    public ClearCartRequestHandlerTest()
    {
        _unitOfWorkMock = Substitute.For<IUnitOfWork>();
        _cartRepositoryMock = Substitute.For<ICartRepository>();

        _clearCartItemRequestHandler = new ClearCartRequestHandler(_cartRepositoryMock, _unitOfWorkMock);
    }

    [Fact]
    public async Task ClearCartRequestHandler_Should_ReturnSuccess_When_ThereIsNoError()
    {
        // Arrange
        ...
        // Act
        Result result = await _clearCartItemRequestHandler.Handle(request, CancellationToken.None);

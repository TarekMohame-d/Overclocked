...
public sealed partial class AuthenticationService
{
    public async Task<Result<AuthResponse>> RefreshTokenAsync(RefreshTokenRequest request, CancellationToken cancellationToken)
    {
        ...
        claims = tokenReaderService.GetClaimsFromToken(request.AccessToken);
        ...
        var isRefreshTokenValid = await refreshTokenService
            .VerifyRefreshTokenAsync(userId, deviceId, request.RefreshToken, cancellationToken);

        if (!isRefreshTokenValid)
            return Result<AuthResponse>.Failure(Errors.InvalidRefreshToken);

        IEnumerable<RolePermission> rolePermissions = await rolePermissionsRepository
            .WhereAsync(x => x.RoleId == user.RoleId, cancellationToken: cancellationToken);

        IEnumerable<string> permissions = rolePermissions
            .Select(x => ((PermissionType)x.PermissionId).ToString());
        ...
        (var refreshToken, DateTime expiredAt) =
            await refreshTokenService.UpdateRefreshTokenAsync(userId, deviceId, cancellationToken);

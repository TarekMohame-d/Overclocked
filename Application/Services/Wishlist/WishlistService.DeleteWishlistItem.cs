...
{
    public async Task<Result> DeleteWishlistItemAsync(Guid userId, Guid productId, CancellationToken cancellationToken)
    {
        ...
        await wishlistItemRepository.DeleteWhereAsync(
            x => x.ProductId == productId && x.WishlistId == wishlist.Id,
            cancellationToken);

...
{
    public async Task<Result<IEnumerable<CategoryListResponse>>> GetAllCategoriesAsync(GetAllCategoriesRequest request,
        CancellationToken cancellationToken)
    {
        IEnumerable<CategoryListResponse> result = [];
        IEnumerable<Domain.Entities.Category> categories =
            await categoryRepository.GetAllAsync(cancellationToken: cancellationToken);
